...
- [x] 2.4 Playwright fallback: `playwright-core` + helper `fetchWithPlaywright(url)` con headers + timeout 15s, integration en adapters si Cheerio 0 resultados
- [x] 2.5 WAF handling: detectar 403/captcha, marcar `enabled=false` temporal, log + métrica, reintento con backoff
- [x] 2.6 `hashPayload` + diff `priceHistory/statusHistory` + `delisted` ya existente, verificar con ingest real + tests
...
- [x] 3.3 Frontend: `MapView` usa clusters cuando `total>500` y `zoom<12`, click cluster → `zoomIn`, test e2e
- [x] 3.4 Tests integración cluster: bbox Madrid zoom 6 → clusters, zoom 12 → points

## 4. Auth + Favoritos/Alertas (favorites-alerts + property-catalog v2)

- [x] 4.1 Instalar `next-auth@5` + `PrismaAdapter`, config `auth.ts` con email provider + `auth.config`, middleware protegiendo `/api/saved-searches`
- [x] 4.2 APIs `GET/POST/DELETE /api/saved-searches` + `GET/POST/DELETE /api/favorites` (401 sin sesión, scoping por userId, idempotencia)
- [x] 4.3 Lógica alertas: hook post-ingest `evaluateAlerts()` → `Alert new_listing` y `price_drop >5%`, `GET /api/alerts` + `PATCH /api/alerts/:id/read`
- [x] 4.4 UI: header campana con badge no leídas, `/saved-searches` CRUD, botón `Alertar` en Filters, `Favorite` heart en cards/ficha
- [x] 4.5 Tests: unit `favorites` idempotencia, integration `saved-search` scoping, e2e login→guardar búsqueda→alerta
...
- [x] 5.4 Tests: health 503 mock, metrics scrape, slack webhook mock
...
- [x] 6.4 Docs `README` deploy local (`docker compose up`) y prod Vercel + envs

## 7. Calidad y verificación

- [x] 7.1 `npm run test:coverage` ≥65% + `npx playwright test` 6+ tests (incl. cluster y alertas) + `npx prisma validate`
- [x] 7.2 `npm run build` y `npx openspec validate whiterock-v2 --strict` verde, archivar
