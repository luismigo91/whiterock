...
El sistema SHALL persistir `Property` con al menos: `id` (UUID), `externalId`, `servicer` (enum), `title`, `description`, `price` (cents), `propertyType` (piso, casa, local, suelo, garaje, nave, etc.), `status` (disponible, reservado, vendido, delisted), `address` (raw + normalized), `province`, `municipality`, `postalCode`, `latitude`, `longitude`, `areaM2`, `rooms`, `bathrooms`, `yearBuilt`, `energyCert`, `photos[]`, `sourceUrl`, `createdAt`, `updatedAt`, `lastSeenAt`.
